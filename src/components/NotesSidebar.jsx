...
export default function NotesSidebar({
  ...
  const [tagInputNoteId, setTagInputNoteId] = useState(null)

  const handleTagKeyDown = (event, noteId) => {
    if (event.key !== 'Enter') return
    event.preventDefault()
    const value = tagInput.trim()
    if (value) {
      onAddTag(noteId, value)
      setTagInput('')
    }
  }
    ...
    <div className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm">
      ...
      {!collapsed && allTags.length > 0 && (
        <div className="border-b border-slate-200 bg-slate-50 px-3 py-2">
          <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500">Filter by tag</p>
      ...
      <ul className="flex-1 space-y-1 overflow-auto p-2">
        {notes.map((note) => {
          ...
          const showTagInput = tagInputNoteId === note.id
          return (
            <li key={note.id}>
              <div
              ...
              >
                ...
                {!collapsed && note.tags.length > 0 && (
                  <div className="mt-1.5 flex flex-wrap gap-1">
                    {note.tags.map((tag) => (
                      <span
                        key={tag}
                        className="group flex items-center gap-0.5 rounded-full bg-slate-300 px-2 py-0.5 text-[10px] font-medium text-slate-700"
                      >
                        #{tag}
                        <button
                          type="button"
                  ...
                  <div className="mt-2">
                    {showTagInput ? (
                      <input
                        ...
                        onBlur={() => {
                          setTagInputNoteId(null)
                          setTagInput('')
                        }}
                        placeholder="Add tag, press Enter"
                        className="w-full rounded border border-slate-300 bg-white px-2 py-1 text-xs outline-none ring-slate-300 focus:ring-2"
                      />
                    ) : (
                      <button
                        type="button"
                        className="text-xs text-slate-400 hover:text-slate-600"
                        onClick={() => setTagInputNoteId(note.id)}
                      >
                        + Add tag
                      </button>
                    )}
